...
public static void main(String[] args) {
        ...
        Utils.afficheListeArticle(Utils.find(articles,"Mixeur",true));
...
class Utils{
    public static Article[] find(Article[] biblio, Categorie cat){
        return Arrays.stream(biblio)
            .filter(article -> article.cat == cat)
            .toArray(Article[]::new);
    }

    public static Article[] find(Article[] biblio, double prixMax){
        return Arrays.stream(biblio)
            .filter(article -> article.prix <= prixMax)
            .toArray(Article[]::new);
    }

    public static Article[] find(Article[] biblio, String motCle, boolean addDescription){
        return Arrays.stream(biblio)
        .filter(article -> article.nom.contains(motCle) || addDescription ? article.description.contains(motCle): false)
        .toArray(Article[]::new);
